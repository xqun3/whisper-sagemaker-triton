...
async def send_whisper(whisper_prompt, audio_data, model_name, padding_duration=15):
...
async def process(
    ...
    text, duration = await send_whisper(whisper_prompt, audio_data, model_name)
...
async def invocations(request: Request):
    try:
        ...
        text = await process(opt.language, opt.repo_id, opt.decoding_method, opt.whisper_prompt, opt.audio_data)
